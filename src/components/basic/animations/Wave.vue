  ...
  <div class="wave-main">
    <svg class="preview-waves" xmlns="http://www.w3.org/2000/svg" xmlns:xlink="http://www.w3.org/1999/xlink"
      ...
      <g class="preview-parallax">
        <use xlink:href="#gentle-wave" x="48" y="0" :fill=getColor(0)></use>
        <use xlink:href="#gentle-wave" x="48" y="3" :fill=getColor(1)></use>
        <use xlink:href="#gentle-wave" x="48" y="5" :fill=getColor(2)></use>
        <use xlink:href="#gentle-wave" x="48" y="7" :fill=getColor(3)></use>
      </g>
    </svg>
  </div>
</template>
...
export default {
  ...
  computed: {
    color() {
      return getRgbNumList(this.$store.getters.themeManager.getVarItem('--v-theme-background'))
    }
  },
  methods: {
    getColor(id) {
